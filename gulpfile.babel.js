...
export function watch() {
  gulp.watch(paths.emails, gulp.series(buildTemplates, devMjml, reload));
  gulp.watch(paths.templates, gulp.series(buildTemplates, devMjml, reload));
  gulp.watch(paths.data, gulp.series(buildTemplates, devMjml, reload));
  gulp.watch(paths.imgs, gulp.series(optimImg, reload));
}

//Optimize imgs, build template and output template as html
gulp.task("build", gulp.series(clean, optimImg, buildTemplates, buildMjml));
gulp.task("build:unminified", gulp.series(clean, optimImg, buildTemplates, devMjml));

//Run build task and create server, reload on change
gulp.task("devTask", gulp.series(clean, buildTemplates, devMjml));
gulp.task("dev", gulp.series("devTask", gulp.parallel(serve, watch)));
